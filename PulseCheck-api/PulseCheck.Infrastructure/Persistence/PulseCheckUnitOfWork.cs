...
public sealed class PulseCheckUnitOfWork(PulseCheckDbContext dbContext) : IPulseCheckUnitOfWork
{
    ...
    public async Task<IReadOnlyList<TransformationalLeaderSession>> GetActiveTransformationalLeaderSessionsByCompanyIdsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (normalizedCompanyIds.Count == 0)
        {
            return [];
        }
        ...
        return sessions
            .Where(item => normalizedCompanyIds.Contains(item.CompanyId.Trim()))
            .ToArray();
    }
    ...
    public Task<TransformationalLeaderAssignment?> GetTransformationalLeaderAssignmentByCompanyIdAsync(string companyId, CancellationToken cancellationToken)
        => dbContext.TransformationalLeaderAssignments.FirstOrDefaultAsync(item => item.CompanyId == companyId.Trim(), cancellationToken);
