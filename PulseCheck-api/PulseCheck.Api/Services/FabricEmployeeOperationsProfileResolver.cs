    ...
    ILogger<FabricEmployeeOperationsProfileResolver> logger) : IEmployeeOperationsProfileResolver
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            command.Parameters.Add("@companyId", SqlDbType.NVarChar, 80).Value = normalizedEmployeeId;
            ...
            return new EmployeeOperationsProfile(
                ReadString(reader, "company_id"),
                ReadString(reader, "operation"),
                ReadString(reader, "status"),
                ReadString(reader, "leader_company_id"),
    ...
    public async Task<IReadOnlyList<TransformationalLeaderCandidate>> GetTransformationalLeaderCandidatesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            command.CommandText = """
                SELECT
                    CAST([company_id] AS nvarchar(80)) AS [company_id],
                ...
                ORDER BY CAST([full_name] AS nvarchar(180)), CAST([company_id] AS nvarchar(80));
                """;
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                var companyId = ReadString(reader, "company_id");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var companyId = ReadString(reader, "company_id");
                if (string.IsNullOrWhiteSpace(companyId) || lookup.ContainsKey(companyId))
                {
                    continue;
                }

                lookup[companyId] = new EmployeeReportFields(
    ...
    private string BuildQuery()
    {
        return """
            SELECT TOP (1)
                CAST(employee.[company_id] AS nvarchar(80)) AS [company_id],
                CAST(employee.[operation] AS nvarchar(180)) AS [operation],
                CAST(employee.[status] AS nvarchar(80)) AS [status],
                CAST(employee.[leader_company_id] AS nvarchar(80)) AS [leader_company_id],
            ...
            OUTER APPLY (
                ...
                WHERE LTRIM(RTRIM(CAST(leaderLookup.[company_id] AS nvarchar(80)))) =
                      LTRIM(RTRIM(CAST(employee.[leader_company_id] AS nvarchar(80))))
            ) leader
            WHERE (LTRIM(RTRIM(@companyId)) <> N''
                    AND LTRIM(RTRIM(CAST(employee.[company_id] AS nvarchar(80)))) = LTRIM(RTRIM(@companyId)))
            ...
            ORDER BY
                CASE
                    WHEN LTRIM(RTRIM(@companyId)) <> N''
                     AND LTRIM(RTRIM(CAST(employee.[company_id] AS nvarchar(80)))) = LTRIM(RTRIM(@companyId)) THEN 0
                    ELSE 1
                END,
                CASE
                ...
                CASE
                    ...
                    ELSE 1
                END,
                CAST(employee.[company_id] AS nvarchar(80));
            """;
    }
    ...
    private string BuildReportFieldsQuery()
    {
        return """
            ...
            INNER JOIN OPENJSON(@employeeIdsJson) employeeIds
                ON LTRIM(RTRIM(CAST(employee.[company_id] AS nvarchar(80)))) =
