...
export function TransformationalLeadersTab({
  ...
  savingCompanyId: string | null
  onSaveAssignment: (companyId: string, operations: string[]) => Promise<void>
  onClearAssignment: (companyId: string) => Promise<void>
}) {
  ...
  function updateDraft(companyId: string, operations: string[]) {
    setDraftAssignments((current) => ({ ...current, [companyId]: uniqueOperations(operations) }))
  }
  ...
  async function saveDraft(leader: TransformationalLeaderCandidate) {
    ...
    await onSaveAssignment(leader.companyId, operations)
    setDraftAssignments((current) => {
      const next = { ...current }
      delete next[leader.companyId]
      return next
    })
  }

  async function clearAssignment(leader: TransformationalLeaderCandidate) {
    await onClearAssignment(leader.companyId)
    setDraftAssignments((current) => {
      const next = { ...current }
      delete next[leader.companyId]
      return next
          ...
          <div className="grid gap-3 border-b border-[#d7e8ee] bg-[#fbfdfe] p-4 lg:grid-cols-[minmax(260px,1fr)_minmax(180px,260px)_minmax(150px,220px)]">
            <label className="grid gap-1.5">
              ...
              <input
                ...
                placeholder="Nombre, Company ID o email"
              />
            </label>
            <label className="grid gap-1.5">
            ...
            <table className="w-full table-fixed border-collapse">
              <thead>
                <tr className="bg-[#f4fbfd] text-left text-[11px] font-black uppercase tracking-[0.14em] text-[#6b8590]">
                  ...
                  <th className="w-[13%] border-b border-[#d7e8ee] px-4 py-3">Company ID</th>
              ...
              <tbody>
                {filteredLeaders.length === 0 ? (
                  ...
                  const effectiveOperations = getEffectiveOperations(leader, draftAssignments)
                  const isSaving = savingCompanyId === leader.companyId
                  return (
                    <tr key={leader.companyId} className="border-b border-[#e1edf2] align-middle">
                      <td className="px-4 py-3">
                        <div className="grid grid-cols-[38px_minmax(0,1fr)] items-center gap-3">
                          <div className="grid h-9 w-9 place-items-center rounded-2xl bg-[#eff9fc] text-xs font-black text-[#00758d]">
                            {getInitials(leader.fullName || leader.companyId)}
                          </div>
                      ...
                      <td className="truncate px-4 py-3 text-sm font-bold text-[#274957]">{leader.companyId}</td>
                      ...
                      <td className="px-4 py-3">
                        <OperationMultiSelect
                          ...
                          onChange={(operations) => updateDraft(leader.companyId, operations)}
                        />
                      </td>
                      <td className="px-4 py-3">
...
function getEffectiveOperations(leader: TransformationalLeaderCandidate, drafts: Record<string, string[]>) {
  return drafts[leader.companyId] ?? getAssignedOperations(leader)
}
...
function exportLeadersCsv(leaders: TransformationalLeaderCandidate[], drafts: Record<string, string[]>) {
  const rows = [
    ['Full name', 'Corporate email', 'Company ID', 'Job title code', 'Status', 'Current operation', 'Assigned operation', 'Client', 'Department'],
    ...leaders.map((leader) => [
      leader.fullName,
      leader.corporateEmail,
      leader.companyId,
