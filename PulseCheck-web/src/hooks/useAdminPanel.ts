...
export function useAdminPanel(token: string, csrfToken: string, activeTab: Tab, role: string) {
  ...
  const [savingTransformationalLeaderCompanyId, setSavingTransformationalLeaderCompanyId] = useState<string | null>(null)
  ...
  async function saveTransformationalLeaderAssignment(companyId: string, operations: string[]) {
    const normalizedCompanyId = companyId.trim()
    const normalizedOperations = uniqueStrings(operations)
    if (!normalizedCompanyId || normalizedOperations.length === 0) {
      return
    }

    try {
      setSavingTransformationalLeaderCompanyId(normalizedCompanyId)
      setError(null)
      ...
      setTransformationalLeaderOptions((current) => ({
        ...
        leaders: current.leaders.map((leader) => (
          leader.companyId === updatedLeader.companyId ? updatedLeader : leader
        )),
      }))
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'No fue posible guardar la asignacion del TL.')
    } finally {
      setSavingTransformationalLeaderCompanyId(null)
    }
  }

  async function clearTransformationalLeaderAssignment(companyId: string) {
    const normalizedCompanyId = companyId.trim()
    ...
    try {
      setSavingTransformationalLeaderCompanyId(normalizedCompanyId)
      setError(null)
      const response = await authorizedFetch(`${apiBaseUrl}/api/transformational-leaders/assignments/${encodeURIComponent(normalizedCompanyId)}`, {
        method: 'DELETE',
      })
      ...
      setTransformationalLeaderOptions((current) => ({
        ...current,
        leaders: current.leaders.map((leader) => (
          leader.companyId === normalizedCompanyId
  ...
  return {
    ...
    ...adminsDomain,
    transformationalLeaderOptions,
    savingTransformationalLeaderCompanyId,
