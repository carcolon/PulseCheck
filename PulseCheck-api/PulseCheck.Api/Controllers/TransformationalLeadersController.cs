...
public sealed class TransformationalLeadersController(TransformationalLeaderService transformationalLeaderService) : ControllerBase
{
    ...
    [HttpDelete("assignments/{companyId}")]
    public async Task<IActionResult> DeleteAssignment(string companyId, CancellationToken cancellationToken)
    {
        var deleted = await transformationalLeaderService.DeleteAssignmentAsync(companyId, cancellationToken);
