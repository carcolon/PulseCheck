    ...
    IDateTimeProvider dateTimeProvider)
{
    public async Task<TransformationalLeaderOptionsDto> GetOptionsAsync(CancellationToken cancellationToken)
    {
        ...
        var assignmentsByCompanyId = assignments.ToDictionary(item => item.CompanyId, StringComparer.OrdinalIgnoreCase);
        ...
        var leaderDtos = leaders
            .GroupBy(item => item.CompanyId, StringComparer.OrdinalIgnoreCase)
            .Select(group => group.First())
            .OrderBy(item => item.FullName)
            .ThenBy(item => item.CompanyId)
            .Select(item =>
            {
                assignmentsByCompanyId.TryGetValue(item.CompanyId, out var assignment);
    ...
    {
        var companyId = NormalizeRequiredPlainText(request.CompanyId, 80, "Company ID");
        ...
        var leaders = await GetActiveLeadersAsync(cancellationToken);
        var leader = leaders.FirstOrDefault(item => string.Equals(item.CompanyId, companyId, StringComparison.OrdinalIgnoreCase))
        ...
        var assignment = await unitOfWork.GetTransformationalLeaderAssignmentByCompanyIdAsync(companyId, cancellationToken);
    ...
    public async Task<bool> DeleteAssignmentAsync(string companyId, CancellationToken cancellationToken)
    {
        var normalizedCompanyId = NormalizeRequiredPlainText(companyId, 80, "Company ID");
        var assignment = await unitOfWork.GetTransformationalLeaderAssignmentByCompanyIdAsync(normalizedCompanyId, cancellationToken);
        if (assignment is null)
        {
            return false;
        }
