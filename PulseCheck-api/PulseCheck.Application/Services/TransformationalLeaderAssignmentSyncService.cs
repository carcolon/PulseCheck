    ...
    IDateTimeProvider dateTimeProvider)
{
    public async Task<TransformationalLeaderAssignmentSyncResult> SyncAsync(CancellationToken cancellationToken)
    {
        ...
        var activeLeaderCompanyIds = activeLeaders
            .Select(item => item.CompanyId.Trim())
        ...
        var cachedByCompanyId = cachedLeaders.ToDictionary(item => item.CompanyId, StringComparer.OrdinalIgnoreCase);
        var addedLeaders = 0;
        var updatedLeaders = 0;

        foreach (var leader in activeLeaders
                     .Where(item => !string.IsNullOrWhiteSpace(item.CompanyId))
                     .GroupBy(item => item.CompanyId.Trim(), StringComparer.OrdinalIgnoreCase)
                     .Select(group => group.First()))
        {
            if (!cachedByCompanyId.TryGetValue(leader.CompanyId.Trim(), out var cached))
            {
                cached = new TransformationalLeaderCandidateCache
                {
                    Id = Guid.NewGuid(),
                    CompanyId = leader.CompanyId.Trim(),
                    CreatedAtUtc = now
                };
        ...
        foreach (var cached in cachedLeaders.Where(item => item.IsActive && !activeLeaderCompanyIds.Contains(item.CompanyId.Trim())))
        {
        ...
        var invalidAssignments = assignments
            .Where(item => !activeLeaderCompanyIds.Contains(item.CompanyId.Trim()))
            .ToArray();
        ...
        var invalidCompanyIds = invalidAssignments.Select(item => item.CompanyId).ToArray();
        var activeSessions = await unitOfWork.GetActiveTransformationalLeaderSessionsByCompanyIdsAsync(invalidCompanyIds, now, cancellationToken);
